...
router.post('/login', [
    ...
    try {
        ...
        if (!admin) {
         ...
         if (admin.endDate && moment(admin.endDate).isBefore(moment(), 'day')) {
        ...
        if (passwordMatch) {
            const token = jwt.sign
                ({ username: admin.username, role: 'counterAdmin' }, SUPERADMIN_JWT_SECRET,);
...
router.post('/resetPassword/:resetToken', [
    ...
    try {
        ...
        const admin = await CounterAdmin.findOne({ resetToken, resetTokenExpires: { $gt: Date.now() } });
